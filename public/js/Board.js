...
export class Board {
  ...
  displayMessage(lines) {
    ...
    // Calculate accurate estimated duration accounting for throttling
    // Tiles may wait in ready queue due to maxConcurrentTiles limit
    const slotEndTimes = []; // Tracks when each concurrent slot becomes free
    let actualLastCompletion = 0;

    for (const flip of pending) {
      // How many steps for this tile
      const physics = this.physicsEngine.calculateFlip(
        flip.tile.currentChar,
        flip.targetChar,
        { motorVariance: this.config.physics?.motorVariance ?? 8 }
      );
      
      // Actual step timing includes min durations from _beginStep:
      // topDuration = max(18, stepCadence * 0.44)
      // bottomDuration = max(20, stepCadence * 0.56) + finalCatch (on last step)
      const actualStepTime = Math.max(18, Math.round(flip.stepCadence * 0.44)) + 
                             Math.max(20, Math.round(flip.stepCadence * 0.56));
      
      // Total = (all steps × actual step time) + final catch on last step
      const tileDuration = (physics.distance * actualStepTime) + flip.finalCatch;

      // Find earliest slot this tile can use
      let slotTime;
      if (slotEndTimes.length < this.performance.maxConcurrentTiles) {
        // Free slot available immediately at tile's start time
        slotTime = flip.startAt;
      } else {
        // Must wait for a slot to free up
        const earliestFree = Math.min(...slotEndTimes);
        slotTime = Math.max(flip.startAt, earliestFree);
        // Remove the used slot
        const idx = slotEndTimes.indexOf(earliestFree);
        slotEndTimes.splice(idx, 1);
      }

      const completionTime = slotTime + tileDuration;
      slotEndTimes.push(completionTime);
      actualLastCompletion = Math.max(actualLastCompletion, completionTime);
    }

    // Use the throttled completion time as the true estimated duration
    estimatedDuration = actualLastCompletion - startAt;
    ...
    return new Promise((resolve) => {
      ...
      }, Math.max(20000, estimatedDuration * 4));
